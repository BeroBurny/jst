...
exports.default = (version) => __awaiter(void 0, void 0, void 0, function* () {
    if (version) {
    ...
    if (!fs.existsSync(extractDestination)) {
        const downloadedFile = yield downloadMetamaskReleases(filename, downloadUrl);
...
const request = (url) => new Promise((resolve) => {
    https_1.get(url, (response) => {
        if (response.statusCode == 302) {
            https_1.get(response.headers.location, resolve);
        }
        else {
            resolve(response);
        }
...
const downloadMetamaskReleases = (name, url) => 
...
const getMetamaskReleases = (version) => new Promise((resolve, reject) => {
    // eslint-disable-next-line @typescript-eslint/naming-convention
    https_1.get(metamaskReleasesUrl, { headers: { 'User-Agent': 'Mozilla/5.0' } }, (response) => {
        let body = '';
